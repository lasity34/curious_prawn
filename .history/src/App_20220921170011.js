...
import { cuisines } from "./api/Objects/cuisines";
...
const App = () => {
  ...
  const loopedCuisines = cuisineList?.map((prev) => prev.name)
  
  console.log(loopedCuisines)
  useEffect(() => {
  ...
  useEffect(() => {
 const filteredCuisines = places?.cuisine?.filter((prev) => {
     console.log(prev.name)
 return prev.name = cuisineList.name
 })
